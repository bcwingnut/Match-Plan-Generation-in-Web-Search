...
class SAC_PolicyNetwork_MLP(nn.Module):
    def __init__(self, num_inputs, max_steps, num_d_actions, num_c_actions, hidden_size, action_range=1.,
        ...
        :param num_params: continuous action dim
        """
        super(SAC_PolicyNetwork_MLP, self).__init__()
        self.batch_size = batch_size
        self.hidden_size = hidden_size
        self.log_std_min = log_std_min
        self.log_std_max = log_std_max
        ...
        self.param_fc = nn.Sequential(
            nn.Linear(hidden_size + num_d_actions, num_c_actions, bias=True)
        )
        self.action_fc[-1].weight.data.uniform_(-init_w, init_w)
        self.param_fc[-1].weight.data.uniform_(-init_w, init_w)  # init in DDPG
